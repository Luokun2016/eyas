...
<body>
	...
	<div class="rightinfo">
		<form action="${base}project/txTeamCheck2" method="post"
			...
			<tbody>
				...
				<c:forEach items="${pagination.list}" var="data" varStatus="i">
					...
					<tr>
						...
						<td width="150"><a pm="XMGL_ZUSHGL_STUINFOTWO"
							href="${base}project/txTeamCheckStudentView2?user_id=${data.user_id}&&project_id=${projectId}"
		...
		<form action="${base}project/txTeamJudge2" method="post" id="teamCheckForm"
		...
		<input type="hidden" name="flag" id="flag"/>
			<cqu:formcols cols="1">
				<c:if
					test="${role==6&&teamTable.team_status==2&&status==1}">
					<col label="复审意见" full="full">
					<textarea class="simple-editor" name="remark2" id="remark2"
						rows="10" cols="105"></textarea>
					</col>
					<col label="&nbsp;">
					<button type="submit" class="btn btn-primary" id="submit_btn"
						onclick="return judge2('pass')">
						<span class="glyphicon glyphicon-floppy-disk"></span>通过
					</button>
					<button type="submit" class="btn btn-primary" id="submit_btn"
						onclick="return judge2('unpass')">
