...
<body>
	<div class="place">
		...
		<span>位置：</span>
		<ul class="placeul">
			<li><a href="#">添加学习报告</a></li>
		</ul>
	</div>
	<form action="${base}report/txReportSave" method="post"
		...
		<div class="formbody">
			...
			<cqu:formcols cols="1">
				...
				<input type="text" class="form-control input-sm" name="center_date"
					datatype="*"  value="<fmt:formatDate value="${trainPlanMain.m_report_date}"
				...
				<input type="text" class="form-control input-sm" name="end_date"
					datatype="*" value="<fmt:formatDate value="${trainPlanMain.e_report_date}"
								pattern="yyyy-MM-dd HH:mm" />"/>
				</col>
				<col label="结业报告模板">
				...
				<select name="l_report_type" id="l_report_type" class="form-control input-sm">
					...
					<option value="0">中期报告</option>
					<option value="1">结业报告</option>
			    </select>
				</col>				

				<col label="报告描述" full="full">
					<textarea class="simple-editor" name="l_report_des" id="l_report_des" rows="10" cols="105"></textarea>
				</col>
				<col label="附件">
