			...
			$.ajax({
				...
				success : function(data) {
					if (data.flag == "false") {
						...
						$("#userName")[0].validform_checktip = "false";
					} else {
						flagName = true;
					}
				},
...
<body style="background-color: #f8f8f8;">
	<div class="topplace">
		<div class="pull-right" style="height: 40px;">
			<a href="${base}index/login">已有账号 点击登录</a>
		</div>
	</div>
		...
		<div class="formbody">
			...
			<cqu:formcols cols="1">
				...
				<input type="text" name="user_name" class="input-sm form-control"
					id="userName" datatype="uName" onchange="checkName()"
				...
				<input type="password" name="password1" onchange="checkPW()"
					...
					recheck="password" placeholder="请再次输入密码" required="required" />
				</col>
				...
				<input type="tel" name="mobile" class="form-control input-sm "
					id="mobile" datatype="m" required="required" onblur="checkPhone()" />
				</col>
