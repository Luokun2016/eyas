...
<body>
	...
	<form action="${base}project/txApplyEdit" method="post"
		...
		<input type="hidden" name="is_publish" id="is_publish" value="${projectApply.is_publish}"/>
			...
			<cqu:formcols cols="1">
				...
				<select name="subject_cat_id" class="form-control input-sm">
					<c:forEach items="${subCategory}" var="sub">
						<option value="${sub.subject_cat_id}" ${sub.subject_cat_id==projectApply.subject_cat_id?"selected='selected'":"" }>${sub.subject_name}</option>
					</c:forEach>
			    </select>
				</col>				
				...
				<input type="text" class="form-control input-sm" name="phone_number"
					datatype="m" id="phone_number"  value="${projectApply.phone_number}"/>
				</col>
				...
				<table class="table table-bordered" id="fj"
					...
					<tr>
						<td><input type="text" class="form-control input-sm" name="attach_name" value="${projectApply.project_attach}" readonly>
						<input type="hidden" name="attach_file" value="${projectApply.project_attach}"></td>
						<td><a href='${base}project/applyDownLoad?attach_file=${projectApply.project_attach}' id='df' class='tablelink'>下载</a>&nbsp;&nbsp;&nbsp;&nbsp;
