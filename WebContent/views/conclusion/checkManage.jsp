			...
			<tbody>
				...
				<c:forEach items="${pagination.list}" var="data" varStatus="i">
					...
					<tr>
						...
						<td><c:if
								test="${role==4&&data.conclusion_status==0}"><a pm="XXGL_JSBGSHGL_CHECKINIT1"
							href="${base}conclusion/check1?conclusion_id=${data.conclusion_id}"
							class="tablelink">初审</a></c:if> <c:if
								test="${role==6&&data.conclusion_status==2}"><a pm="XXGL_JSBGSHGL_CHECKINIT2"
							href="${base}conclusion/check2?conclusion_id=${data.conclusion_id}"
							class="tablelink">复审</a></c:if> <a pm="XXGL_JSBGWHGL_CHECKVIEW"
							href="${base}conclusion/checkViewC?conclusion_id=${data.conclusion_id}"
