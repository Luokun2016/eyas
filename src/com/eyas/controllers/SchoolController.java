...
public class SchoolController extends BaseController {	
	 ...
	 * 修改学校检测学校名称
	 */
	public void editCheckSchoolName(){
		String schoolId = getAttr("schoolId");
		String schoolName = getAttr("schoolName");
		Record school = Db.findFirst("select school_name as name from schools where school_name=? and school_id != ?",schoolName, schoolId);
		String flag ="false";
		if(school == null){
			flag = "true";
		}
	...
	public void txEditSchool(){
		...
		int s =Db.update("update schools set school_name = ? ,school_type = ?, manager = ?,address = ?, phone =?, remark =? where school_id = ? ", 
				 schoolName, schoolType, manager, address, phone, remark, schoolId );
		if(s != -1){
