...
public class RecordController extends BaseController {
	...
	public void queryProgramme() {
		...
		if(role == 6){ // 6—创新学院管理员
			List<Record> batchList = Db.find("select batch_id, batch_name from batch b where b.batch_id in (select batch_id from learning_record_table)  or b.batch_status=1 order by b.batch_status desc,b.batch_id desc");
			setAttr("batchList", batchList);

			List<Record> projectList = Db.find("select project_id, project_name from project_apply where project_id in (select project_id from learning_record_table)");
			setAttr("projectList", projectList);
			...
			List<Record> collegeList = Db.find("select school_id,school_name from schools where school_id in (select pa.school_id from learning_record_table lr join project_apply pa on lr.project_id=pa.project_id where lr.user_id='"+r.get("user_id")+"')");
			setAttr("collegeSchoolList", collegeList);
